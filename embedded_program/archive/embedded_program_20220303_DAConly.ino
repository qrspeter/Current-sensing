(при обмене через терминал все работает - ЦАП меняет напряжение на выходе, АЦП меняет разрядность и сообщает результат).
...
// ==============
 ...
 // ADC setup
  Wire.beginTransmission(ADC_DRAIN_ADDRESS);// ?? i2c адрес MCP3421= B1101000... "Диапазон адресов А0-А7"
  Wire.write(0b11111); // Настройка в 12 бит - Wire.write(B10011)... 
  Wire.endTransmission();
...
//      while(Serial.available() && i < bufferSize){
