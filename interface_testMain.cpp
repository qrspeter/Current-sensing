...
// std::stod    - Convert string to double https://cplusplus.com/reference/string/stod/
 ...
 // добавить с проверкой на получение ненулевого значени€, а если нулевое - то ничего не делать.
// хот€ почему-то проста€ вставка без проверки ничего не выводит.. отложили на потом.
...
// https://github.com/wxWidgets/wxWidgets/blob/master/src/univ/dialog.cpp
    ...
    // запрашивать результаты измерений выходного напр€жени€ (тестером), например напр€жение при задании на ј÷ѕ 0x000000, 0x888888 и 0xFFFFFF. » затем пересчет уже по этим данным
    // то есть если сейчас расчет идет по двум значени€м (мин и макс), то там еще и ноль. » они также хран€тс€ в виде данных класса, и мен€ютс€ при настройке.
    // может начать пока с мин и макс, а ноль - если потребуетс€. “огда надо будет отдельную функцию писать расчета величины, котора€ бы провер€ла больше или меньше нул€.

    // и м.б. также калибровать ј÷ѕ, чтобы при нуле он показывал ноль.
...
// функци€ чтобы обрабатывать ENTER https://forums.wxwidgets.org/viewtopic.php?f=1&t=46747&p=196346#p196332
