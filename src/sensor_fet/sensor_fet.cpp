/**
 *  áèáëèîòåêà ôóíêöèé ðàáîòû ñ ñåíñîðîì.
 *  Óñòàíàâëèâàåò íàïðÿæåíèå äâóõ ÖÀÏ (çàòâîð è ñòîê), êîíâåðòèðóÿ íàïðÿæåíèå double â îòñ÷åòû ÖÀÏ.
 *  Çàïóñêàåò ÀÖÏ è çàäàåò åãî íàñòðîéêè (âûáîð ÀÖÏ, ðàçðÿäíîñòü, óñèëåíèå)
 *  Ñ÷èòûâàåò îòñ÷åòû ÀÖÏ è êîíâåðòèðóåò èõ â íàïðÿæåíèå (õîòÿ íàì íóæåí áóäåò òîê).
 *  Âêëþ÷àåò-âûêëþ÷àåò ëàçåð ÷åðåç öèôðîâîé âûâîä Àðäóèíî.
 *  À òàêæå óñòàíàâëèâàåò êîíñòàíòû ïåðåñ÷åòà îòñ÷åòîâ â íàïðÿæåíèå è îáðàòíî.
 *
 */
...
#include <iostream>
//#include <wx/filename.h> // äëÿ wxYield
...
int SENSOR_FET::Open(int port)
{
	...
	if(hSerial == INVALID_HANDLE_VALUE) // ïðîâåðêà âûäåëåíèÿ ïîðòà. NULL for 32-bit Win and INVALID_HANDLE_VALUE for 64bit
	{
	...
	SetupComm(hSerial, INPUT_BUFF_SIZE, INPUT_BUFF_SIZE); // çàäàíèå ðàçìåðîâ áóôåðà ïîðòà.
	...
	dcb.fDtrControl = DTR_CONTROL_ENABLE; // èíà÷å ðàáîòàåò òîëüêî ïîñëå âêëþ÷åíèÿ òåðìèíàëà ïîðòà, èäåÿ îòñþäà https://github.com/dmicha16/simple_serial_port/blob/master/simple-serial-port/simple-serial-port/SimpleSerial.cpp
	// õîòÿ ñòðàííî, ïàðàìåòð îòâå÷àåò çà ñáðîñ ïðè âêëþ÷åíèè
	...
	// õîòÿ ... ýòî "Data terminal ready" https://docs.microsoft.com/en-us/windows/win32/api/winbase/ns-winbase-dcb
...
int SENSOR_FET::Set_voltage(terminal  term, double voltage)
{
    ...
    // íóæíà ïðîâåðêà íà ïåðåïîëíåíèå
    if(dac_count < 0)
        dac_count = 0;
...
// äîáàâèòü óñðåäíåíèå â çàïðîñ...  è ïîòîì â ÊÎÏ

// ñáðàñûâàåì, ò.ê. èíîãäà áàéòû êóäà-òî òåðÿþòñÿ è ñêà÷êè ïðîèñõîäÿò ñòóïåíüêîé, à òàê õîòü ðàçîâûì âûáðîñîì.
    ...
    // ðàçðÿäíîñòü 12-14-16-18 -> âû÷èòàåì 12 è äåëèì íà 3, ïîëó÷àåì 0...3, êàê íàäî
    // óñèëåíèå 1-2-4-8 -> ...
    // íîìåð êàíàëà - êàê åñòü, 0...3
    ...
    // 12-14-16-18 áèò ýòî b10000000-b10000100-b10001000-b10001100 (x80/x84/x88/x8C)

    WriteFile(hSerial, &setADC, sizeof(setADC), &bc, NULL);
//    // êîãäà äîäåëàþ ÊÎÏ â êîíòðîëëåðå, õîòÿ ïðîùå ïðîãðàììíî ãäå-òî òóò óñðåäíÿòü:
//    if(averaging == 1)
//        WriteFile(hSerial, &setADC, sizeof(setADC), &bc, NULL);
//    else
//    {
//        WriteFile(hSerial, &setADCav, sizeof(setADCav), &bc, NULL);
//        WriteFile(hSerial, &averaging, sizeof(averaging), &bc, NULL); // // òî åñòü ÊÎÏ setADCav ïðåäïîëàãàåò ÷òî ìê çàïðîñèò åùå îäèí áàéò
//    }
...
double SENSOR_FET::Get_voltage()
{
    ...
    do
    {
        ...
        Beep(523,50);
        // åñëè ñáîé ñîïðîâîæäàåòñÿ èçìåíåíèåì â áàéòå ñòàòóñà - òî ÀÖÏ çàïóñêàåòñÿ ïîâòîðíî, ñî ñáðîñîì, òê áåç íåãî ýòî çàíèìàåò ñåêóíä 10 (!)
        if(!PurgeComm(hSerial, PURGE_TXABORT | PURGE_RXABORT | PURGE_TXCLEAR | PURGE_RXCLEAR))
        {
            MessageBox(NULL, "PurgeComm() failed!", "Error", MB_OK);
            return std::nan("");
        ...
        WriteFile(hSerial, &setADC, sizeof(setADC), &bc, NULL);
        WriteFile(hSerial, &status, sizeof(status), &bc, NULL);
        // íî ïîêà ïî÷åìó-òî ïðîãà çàâèñàåò, à îêîøêî íå ïîÿâëÿåòñÿ.
        ++reRead;

    }
    while(adc.meas_status  != expected_status); // à çà÷åì äâàæäû ïðîâåðÿòü? àíàëîãè÷íàÿ ïðîâåðêà âûøå èäåò âåäü...
...
// Áåç êîððåêöèè íà ñìåùåíèå íóëÿ, òîëüêî ðåçóëüòàò èçìåðåíèÿ ÀÖÏ:
    double result = adc_ref_ * static_cast<double>(raw_adc) / (1 * (static_cast<double> (adc_counts) / adc_res_correction)); // âðåìåííî áåç êîððåêöèè íà óñèëåíèå ,à ìîæåò òàê è ïîíÿòíåå áóäåò....
...
int SENSOR_FET::CheckState() // Âîçâðàùàåò äà/íåò, â çàâèñèìîñòè îò ñîñòîÿíèÿ ïîäêëþ÷åíèÿ.
{
	if(hSerial == INVALID_HANDLE_VALUE) // Ò.å. ñ÷åò÷èê íå ïîäêëþ÷àëñÿ.
        return 0;
	else
        return 1;
}
...
void SENSOR_FET::Close()
{
	if(hSerial != INVALID_HANDLE_VALUE)
	{
		CloseHandle(hSerial); // ïîõîæå íå ñðàáàòûâàåò... - ïðè îòêëþ÷åíèè íå ñáðàñûâàåò.
...
{
	if((port_number > 0) && (port_number < 10) && (hSerial == INVALID_HANDLE_VALUE)) //  òîëüêî åñëè ìîíîõðîìàòîð îòêëþ÷åí
